...
import {selectProtocol} from "./voice_gateway.js";
...
// const djsNetworking = new Networking();
const generateConnectionData = () => {
    return {
        encryptionMode: mode,
        nonce: nonce,
        nonceBuffer: cloneBuffer(nonceBuffer),
        packetsPlayed: 0, // not used by djs
...
export const sendAudio = (data) => {
    // if(last_sent && Date.now() - last_sent > 10) console.log(`Last voice packet sent ${Date.now() - last_sent}ms ago`);
    // const djs = djsNetworking.createAudioPacket(data, generateConnectionData());
    const homeGrown = createAudioPacket(data);
    // if(Buffer.compare(homeGrown, djs) !== 0) debugger;
    send(homeGrown);
    // send(djs);
    packetNumber++;
    last_sent = Date.now();
...
const createAudioPacket = (data) => {
    const packetBuffer = Buffer.alloc(12);
    packetBuffer[0] = 0x80;
    packetBuffer[1] = 0x78;

    packetBuffer.writeUIntBE(packetNumber, 2, 2);
    packetBuffer.writeUIntBE(timestamp, 4, 4);
    packetBuffer.writeUIntBE(ssrc, 8, 4);

    timestamp += (48_000 / 100) * 2;

    packetBuffer.copy(nonceBuffer2, 0, 0, 12);
    return Buffer.concat([packetBuffer, ...encryptOpusPacket(data)]);
}

const encryptOpusPacket = (data) => {
